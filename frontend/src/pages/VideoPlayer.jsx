...
const VideoPlayer = () => {
  ...
  useEffect(() => {
    const fetchVideoPlayerContextData = async () => {
      ...
      setErrorNotice('');
      try {
        // 1. Fetch data from video metadata schemas (automatically increments backend view counter)
  ...
  const handleAddCommentSubmit = async (e) => {
    ...
    try {
      ...
      alert(err.response?.data?.message || 'Authentication token signature required to comment.');
    }
  };
  ...
  const handleUpdateCommentAction = async (commentId, updatedText) => {
    try {
      const response = await API.put(`/comments/${commentId}`, { text: updatedText });
  ...
  // --- CRUD ACTION 3: ERDASE/DELETE RECORD ---
  const handleDeleteCommentAction = async (commentId) => {
    if (!window.confirm('Are you sure you want to remove this comment permanently?')) return;
  ...
  if (pageLoading) return <div className="player-status-banner">Buffering video stream infrastructure...</div>;
  if (errorNotice) return <div className="player-status-banner status-err-box">{errorNotice}</div>;
  if (!video) return <div className="player-status-banner">Video object not found inside system registries.</div>;
    ...
    <div className="player-workspace-split-grid">
      <div className="player-primary-column">
        ...
        {video.channelId && (
          <div className="player-channel-profile-card">
            ...
            <div className="channel-brand-meta-info">
              <Link to={`/channel/${video.channelId._id}`} className="channel-brand-anchor-link">
                <h4>{video.channelId.channelName}</h4>
              </Link>
              <p>{video.channelId.subscribers || 0} subscribers</p>
              <p className="video-description-paragraph-text">{video.description}</p>
            </div>
          </div>
        )}
        ...
        <div className="video-player-comments-section">
          ...
          {user ? (
            <form onSubmit={handleAddCommentSubmit} className="add-comment-creation-form">
              <img src={user.avatar} alt="Active User Avatar" className="comment-author-avatar" />
              <div className="comment-input-action-wrapper">
                ...
                <div className="form-submit-actions-panel">
                  {newCommentText.trim() && (
                    <button type="submit" className="comment-post-submit-action-btn">
                      Comment
                    </button>
                  )}
                </div>
